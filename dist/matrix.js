...
export function parseMatrix(input, maxDim, roundPlaces) {
    if (!Array.isArray(input) || input.length === 0)
        return { error: 'matrix must be a non-empty array of rows (number[][])' };
    if (input.length > maxDim)
        return { error: `matrix has ${input.length} rows; maximum is ${maxDim}` };
    const rows = [];
    let cols = -1;
    for (let i = 0; i < input.length; i++) {
        const row = input[i];
        if (!Array.isArray(row) || row.length === 0)
            return { error: `row ${i} must be a non-empty array of numbers` };
        if (cols === -1)
            cols = row.length;
        else if (row.length !== cols)
            return { error: `row ${i} has ${row.length} entries; expected ${cols} (matrix must be rectangular)` };
        if (cols > maxDim)
            return { error: `matrix has ${cols} columns; maximum is ${maxDim}` };
        const cleanRow = [];
        for (let j = 0; j < row.length; j++) {
            const v = row[j];
            if (typeof v !== 'number' || !Number.isFinite(v))
                return { error: `entry at row ${i} column ${j} is not a finite number` };
 ...
 */
export function determinant(m, roundPlaces) {
    const n = rowsOf(m);
    if (n !== colsOf(m))
        return null;
...
export function inverse(m, roundPlaces) {
    const n = rowsOf(m);
    if (n !== colsOf(m))
        return null;
    // augmented [A | I]
    const aug = m.map((row, i) => {
        const identityRow = Array.from({ length: n }, (_, j) => (j === i ? 1 : 0));
...
export function rref(m, roundPlaces) {
    const a = m.map((row) => [...row]);
    const rows = rowsOf(a);
    const cols = colsOf(a);
    let pivotCol = 0;
    for (let pivotRow = 0; pivotRow < rows && pivotCol < cols; pivotRow++) {
        ...
        for (let c = pivotCol; c < cols; c++)
            a[pivotRow][c] = (a[pivotRow]?.[c] ?? 0) / pivot;
 ...
 */
export function solve(a, b, roundPlaces) {
    const m = rowsOf(a);
    const n = colsOf(a);
    if (b.length !== m)
        return { error: `b has ${b.length} entries; expected ${m} (one per row of A)` };
